...
const CloseDateUpsert = ({ closeDateId }) => {
  ...
  const getCloseDateData = async () => {
    ...
    if (result.ok || (result.status === 200 && data)) {
      ...
      setCompareData({
        title: data?.closeDate?.title,
        description: data?.closeDate?.description,
        start_date: data?.closeDate?.start_date,
        end_date: data?.closeDate?.end_date,
  ...
  return (
    <div className="mainContent">
      ...
      <SuccessPopUpModal
        ...
        redirectPath={
          closeDateId
            ? `/system-configuration/tenant-admin/operations-admin/calendar/close-dates/${closeDateId}/view`
            : '/system-configuration/tenant-admin/operations-admin/calendar/close-dates'
        }
      />
      <CancelModalPopUp
