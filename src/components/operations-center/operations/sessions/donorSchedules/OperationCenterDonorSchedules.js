...
export default function OperationCenterDonorSchedules({ setSessionDataOne }) {
  ...
  const [donorPortalProcedure, setDonorPortalProcedure] = useState([]);
  ...
  const getDonors = async () => {
    try {
      const result = await makeAuthorizedApiRequest(
        'GET',
        `${BASE_URL}/operations/sessions/donors?fetchAll=true&keyword=${accountsSearchText}${
  ...
  const getDonorSchedules = async () => {
    try {
      const result = await makeAuthorizedApiRequest(
        'GET',
        `${BASE_URL}/operations/sessions/shifts/donors-schedules/${id}`
      );
      const data = await result.json();
      if (data?.data) {
        const modifiedSessionDataOne = {
          donor_center: data?.data?.donor_center[0]?.donor_center,
          session_date: data?.data?.donor_center[0]?.session_date,
        };
        setSessionDataOne(modifiedSessionDataOne);
        // const counts = {};
        ...
        const procedureTypesArray = data?.data?.shift_slots
          ...
          ?.map((item) => {
            ...
            return {
              ...
              is_donor_portal:
                item?.procedure_type?.projections?.is_donor_portal_enabled,
            };
          });
        const check = procedureTypesArray?.map((item) => {
          return {
            id: item?.procedure_type?.id,
            donor_portal:
              item?.procedure_type?.projections?.is_donor_portal_enabled,
          };
        });
        setDonorPortalProcedure(check);
  ...
  const handleShiftSlots = async (id, firstSlots) => {
    ...
    try {
      const response = await makeAuthorizedApiRequest(
        'POST',
        `${BASE_URL}/operations/sessions/shifts/procedure-type/slots`,
        JSON.stringify(body)
      );
  ...
  const handleDonorPortal = async (id, firstSlots) => {
    ...
    try {
      const response = await makeAuthorizedApiRequest(
        'POST',
        `${BASE_URL}/operations/sessions/shifts/procedure-type/projection-staff`,
        JSON.stringify(body)
      );
      const data = await response.json();
      if (data.status === 'success') {
        setDonorPortalProcedure((prev) =>
          prev?.map((item) => {
            if (item?.id === data?.data[0]?.procedure_type_id) {
              return {
                id: data?.data[0]?.procedure_type_id,
                donor_portal: data?.data[0]?.is_donor_portal_enabled,
              };
            } else {
              return item;
            }
  ...
  useEffect(() => {}, [donorPortalProcedure]);
  ...
  const handleIsActiveChange = async (e) => {
    ...
    try {
      const response = await makeAuthorizedApiRequest(
        'PATCH',
        `${BASE_URL}/operations/sessions/shifts/projection/staff`,
        JSON.stringify(body)
      );
      const data = await response.json();
      if (data.status === 'success') {
    ...
    <div className="mainContent">
      <div className="plasmaButtons">
        {shiftSlots?.map((item, index) => {
          const checking = donorPortalProcedure
            ?.filter((ele) => ele?.id == item?.procedure_type?.id)
            ?.map((donor) => (donor.donor_portal ? 'green-dot' : 'gray-dot'))
            ?.join('');
          return (
            <button
              onClick={() => {
