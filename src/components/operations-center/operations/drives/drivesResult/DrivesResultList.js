...
import DriveHistoryTableList from './DriveResultTableList';
...
export default function DrivesResultList() {
  const { drive_id: id } = useParams();
  ...
  const BreadCrumbsData = [
    ...
    },
    {
      label: 'Results',
      class: 'active-label',
      link: DRIVES_RESULT_PATH.LIST.replace(':drive_id', id),
    },
  ];
  const tableHeaders = [
  ...
  const getResultApi = async () => {
    try {
      const res = await makeAuthorizedApiRequest(
        'GET',
        BASE_URL +
          `/operations/sessions/${id}/results?operationable_type=${PolymorphicType.OC_OPERATIONS_DRIVES}`
      );
