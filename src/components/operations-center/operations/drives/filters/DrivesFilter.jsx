...
import DynamicForm from './DrivesDynamicForm';
...
function DrivesFilter({
  ...
  const [promotionsOptions, setPromotionsOption] = useState();
  ...
  const getLocations = async () => {
    const accounts = await API.crm.location.getAllLocations();
    const { data } = accounts;
    setLocations([
      ...(data?.data?.map((item) => {
        return { value: item.id, label: item.name };
  ...
  const fetchPromotions = async () => {
    try {
      const response = await makeAuthorizedApiRequest(
        'GET',
        `${BASE_URL}/marketing-equipment/promotions?sort_name=name&sort_order=ASC`
      );
  ...
  const fetchOperationStatus = async () => {
    try {
      const result = await makeAuthorizedApiRequest(
        'GET',
        `${BASE_URL}/booking-drive/operation-status?appliesTo=Drives&status=true&sortName=name&sortOrder=ASC`
      );
  ...
  const getFilters = async () => {
    try {
      const result = await makeAuthorizedApiRequest(
        'GET',
        `${BASE_URL}/filters/single/drives`
      );
      const data = await result.json();
  ...
  const postSelectedFilter = async () => {
    try {
      ...
      const apiPayload = {
        ...
        application_code: PolymorphicType.OC_OPERATIONS_DRIVES,
  ...
  const handleArchiveFilter = async (name) => {
    const result = await makeAuthorizedApiRequest(
      'POST',
      `${BASE_URL}/filters/delete/${deleteId}/drives`
    );
    await result.json();
    if (result.ok || result.status === 200) {
  ...
  return (
    <div className="filterBar crm-filters">
      ...
      <SuccessPopUpModal
        title="Success!"
        message={`${archivedName} is archived.`}
