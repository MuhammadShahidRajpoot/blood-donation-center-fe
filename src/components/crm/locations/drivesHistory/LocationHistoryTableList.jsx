...
export default function LocationHistoryTableList({
  ...
  const getDriveData = async (rowData) => {
    try {
      const result = await makeAuthorizedApiRequestAxios(
        'GET',
        `${BASE_URL}/drives/${rowData.driveid}`
      );
      const data = await result?.data?.data;
      if (data && data?.length > 0) {
        if (data[0]?.account?.is_archived === true) {
          setWarningModalPopUp(true);
          setWarningMessage('You cannot copy it. Account has been archived');
        } else if (data[0]?.account?.is_active === false) {
          setWarningModalPopUp(true);
          setWarningMessage(
            `${data[0]?.account?.name} is not an active account`
          );
        } else if (userData?.is_recruiter === true) {
          if (data[0]?.account?.recruiter?.id == userData?.id) {
            setAccountId(data[0]?.account?.id);
