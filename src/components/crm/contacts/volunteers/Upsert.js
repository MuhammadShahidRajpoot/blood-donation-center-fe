...
function VolunteerUpsert() {
  ...
  const getCustomDataFields = async () => {
    try {
      const response = await makeAuthorizedApiRequest(
        'GET',
        `${BASE_URL}/system-configuration/organization-administration/custom-fields/data?custom_field_datable_id=${volunteerID}&custom_field_datable_type=${PolymorphicType.CRM_CONTACTS_VOLUNTEERS}`
      );
      const data = await response.json();
  ...
  const onSubmit = async (data, e, redirect = true) => {
    try {
      ...
      const customFieldDatableType = PolymorphicType.CRM_CONTACTS_VOLUNTEERS; // You can change this as needed
      let resulting;
  ...
  const resetForm = () => {
    ...
    setPrimaryPhone(0);
    navigate(
      `/crm/contacts/volunteers/${duplicateConfirmation?.duplicate?.[0]?.volunteer_id}/view`
    );
  };
      ...
      <SuccessPopUpModal
        ...
        onConfirm={() => {
          if (closeAfterFinish.current) {
            navigate(
              duplicateId
                ? `/crm/contacts/volunteers/${duplicateId}/view/duplicates`
                : -1
            );
          }
        }}
