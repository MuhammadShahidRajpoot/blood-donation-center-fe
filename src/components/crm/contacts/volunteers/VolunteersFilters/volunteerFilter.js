...
function ContactVolunteerFilters({
  ...
  let inputTimer = null;
  useEffect(() => {
    getVolunteerSeedData();
  }, []);
  const getVolunteerSeedData = async () => {
    try {
      const result = await makeAuthorizedApiRequest(
        'GET',
        `${BASE_URL}/contact-volunteer/upsert/seed-data`
      );
  ...
  useEffect(() => {
    setSelectedOptionsData({ ...filterFormData });
  }, [filterFormData]);
  useEffect(() => {
    reinitializeState();
