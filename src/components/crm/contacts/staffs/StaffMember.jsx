...
const StaffMember = ({ refreshData, primaryRole }) => {
  const params = useParams();
  const [staff, setStaff] = React.useState(null);
  const [staffRolesState, setStaffRoles] = useState([]);

  const fetchStaff = async () => {
    try {
      const [staffRes, staffRoles] = await Promise.all([
        fetchData(
          `/contact-staff/${params?.staff_id ?? params?.staffId ?? params?.id}`
        ),
        fetchData(
          `/contact-staff/${
      ...
      setStaff({ ...staffRes?.data, role: staffRole?.role_id });
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const primary_role = staffRolesState?.find((role) => role?.is_primary);
